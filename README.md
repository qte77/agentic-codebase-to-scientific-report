...
|--------|---------|-------------|----------|
| **Automated complete Pipeline** | `make all` | Full pipeline: ingest → analyze → synthesize → validate → PDF | Recommended for most users |
...
- **Section Generation**: AI-driven creation of pandoc-ready sections (title/abstract, 8 numbered content sections, BibTeX bibliography; List of Figures/Tables auto-generated by pandoc)
...
│   ├── sections/           # analysis.yaml, report sections, references.bib
│   ├── assets/             # Rendered diagrams and images
│   ├── validation-report.md # Quality scores
│   └── report.pdf          # Final assembled PDF
├── pyproject.toml          # Minimal manifest (report name/version)
├── AGENTS.md               # Pipeline orchestration docs
...
- [x] LaTex option (XeLaTeX via `scripts/writeup/run-pandoc.sh`)
- [x] Repo ingestion (Repomix + Graphify)
- [ ] Use Github API for remote repo access
