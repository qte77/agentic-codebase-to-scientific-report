...
- Restructured `README.md` to the qte77 README doc-structure canon: `## Purpose` →
  `## What` (rewritten as tight bullets), `## Quick start` → `## How`, `##
  Documentation` → `## Refs` (links only, repositioned after `## Why`); added a
  `## Why` section and a License/Version/CI badges row; moved `## Project
  structure` into `CONTRIBUTING.md` and dropped the `## Development` pointer
  section, now covered by CONTRIBUTING's own Quick commands table (#28).
